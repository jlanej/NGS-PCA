#!/usr/bin/env python3
"""Concordance of the two runs' PC sets as subspaces, not as ordered lists.

Run by 04b_fast_mode_report.sh inside the eval container (numpy arrives with
matplotlib there); the report assembler embeds what this writes.
...
Outputs, written beside this script:
  ...
  subspace_angles.tsv     for nested leading-k subspaces: canonical
                          ...
                          angle), that angle in degrees, and the mean cos.
                          cos ~ 1 at every k means the k-dimensional leading
                          subspaces are interchangeable.
  pc_containment.tsv      per normal PC: R^2 of its projection onto the span
                          of ALL fast PCs (and the same-index leading-k
                          span), its best-matching fast PC and that |r| -
                          rotation shows as containment ~ 1 with a poor
                          matched-index |r|.
  pc_crosscorr.png        |correlation| heatmap, normal x fast - rotations
                          appear as small blocks around the diagonal, swaps
                          as off-diagonal dots.
...
def main():
    ...
    parser.add_argument("--normal", required=True, help="svd.pcs.txt from the standard run")
    parser.add_argument("--fast", required=True, help="svd.pcs.txt from the fast run")
    ...
    lines = ["k\tmin_cos\tlargest_angle_deg\tmean_cos"]
    angle_rows = []
    for kk in ks:
        ...
        angle_rows.append((kk, worst, angle, float(cosines.mean())))
        lines.append(f"{kk}\t{worst:.6f}\t{angle:.3f}\t{cosines.mean():.6f}")
    ...
    with open(os.path.join(args.out_dir, "pc_containment.tsv"), "w") as out:
        out.write("PC\tmatched_abs_r\tbest_match_fast_PC\tbest_abs_r\t"
                  "containment_in_all_fast\tcontainment_in_leading_same_k\n")
        for i in range(k):
    ...
    try:
        ...
        ax.set_xlabel("fast PC")
        ax.set_ylabel("normal PC")
        ax.set_title("|correlation|, normal x fast - rotation blocks hug the diagonal")
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    diag = np.diag(absR)
    md = ["# PC set concordance, normal vs fast", ""]
    md.append(f"{len(shared)} shared samples; {k} PCs per run.")
    md.append("")
    md.append("| leading k | largest principal angle | min canonical correlation |")
    md.append("|---|---|---|")
    for kk, worst, angle, _ in angle_rows:
        md.append(f"| {kk} | {angle:.2f}° | {worst:.6f} |")
    md.append("")
    ...
    md.append(f"- Containment of each normal PC in the span of all {k} fast PCs: "
